...
def unf_zfactor_DAK(p_MPaa, t_K, ppc_MPa, tpc_K):
    ...
    keff_A = (0.3265 - 1.0700 / tpr - 0.5339 / tpr**3 + 0.01569 / tpr ** 4 - 0.05165 / tpr ** 5)
    keff_B = (0.5475 - 0.7361 / tpr + 0.1844 / tpr ** 2)
    keff_C = - 0.1056 * (-0.7361 / tpr + 0.1844 / tpr ** 2)
    _2_zeros = np.zeros(2)
    ...
    def f(variables):
        ...
        _abc2 = 1 + keff_A * ropr + keff_B * ropr ** 2 + keff_C * ropr ** 5
        # _abc = np.polyval(ABC_poly, ropr)
        func[1] = -z + _abc2 + 0.6134 * (1 + 0.7210 * ropr ** 2) * (ropr ** 2 / tpr ** 3) * np.exp(-0.7210 * ropr ** 2)
        return func
