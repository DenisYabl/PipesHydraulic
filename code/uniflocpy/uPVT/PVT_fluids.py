...
class FluidBlackOil:
    ...
    def __init__(self, gamma_oil=0.86, gamma_gas=0.6, gamma_wat=1.0, rsb_m3m3=200.0, gamma_gassp=0, y_h2s=0, y_co2=0,
        ...
        fl = [k for k in self.__dict__.keys() if k not in ("t_c", "p_bar")]
        self.field_list = fl
        ...
        tmp_lst = self.field_list + ["field_list"]
        rez_fl = [k for k in self.__dict__.keys() if not k in tmp_lst]
        self.calc_properties_list = rez_fl
    ...
    def as_named_tuple(self):
        fluid_nt = namedtuple('FluidBlackOil_Tuple', self.field_list)
        kv = {k:self.__dict__[k] for k in self.field_list}
        return fluid_nt(**kv)

Standing_cache = dict(Miss=0, Hit=0)
...
class FluidStanding(FluidBlackOil):  # TODO после проверки свойств добавить их в FluidMcCain
    ...
    def calc(self, p_bar, t_c):
        ...
        """
        # arg = (self.as_named_tuple(), (p_bar, t_c))
        # if arg in Standing_cache:
        #     Standing_cache["Hit"]+=1
        #     rez = Standing_cache[arg]
        #     for k, v in rez.items():
        #         self.__dict__[k] = v
        #     return 1
        # Standing_cache["Miss"]+=1
        ...
        # rez = dict()
        # for k in self.calc_properties_list:
        #     rez[k] = self.__dict__[k]
        # Standing_cache[arg] = rez
        return 1
