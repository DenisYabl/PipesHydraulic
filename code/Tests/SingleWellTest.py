...
oil_params = {
    ...
    "wellopVolumeWater": 70.7, #Обводненность нефти, исходные данные
...
outlets = dict(wellhead = vrtxs.HE2_Boundary_Vertex('Q', 104 * 855 / 86400))
...
#Пласт
G.add_edge('PLAST_88', 'ZABOI_ZONE', obj=HE2_Plast(productivity = 0.515, fluid=fluid))  #+
#Инклинометрия до устья
G.add_edge('Pump_outlet', 'wellhead',
           obj=HE2_OilPipe([382.13], [2367.87], [0.125 * real_diam_coefficient], [roughness]))
#Инклинометрия от забоя до приема насоса
G.add_edge('ZABOI_ZONE', 'Pump_intake',
           obj=HE2_OilPipe([652.87], [230.96], [0.143 * real_diam_coefficient], [5 * roughness]))
#Насос
G.add_edge('Pump_intake', 'Pump_outlet',
           obj=HE2_WellPump(full_HPX=pump_curves, model="ЭЦН5-125-2500", fluid=fluid, IntDiameter=0.12, frequency=48))
